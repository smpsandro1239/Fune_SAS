...
import { Sparkles, User, LogOut, Building2, Loader2, Menu } from 'lucide-react';
...
export default function Navbar({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  const initials = user
    ? user.name.split(' ').filter(Boolean).map((p) => p[0]).join('').substring(0, 2).toUpperCase()
    : '??';
  ...
  return (
    <header className="h-16 border-b border-navy-700/60 bg-navy-900/90 backdrop-blur-md fixed top-0 left-0 right-0 z-50 px-4 md:px-6 flex items-center justify-between">
      <div className="flex items-center space-x-3">
        {onMenuToggle && (
          <button
            onClick={onMenuToggle}
            className="md:hidden p-2 rounded-lg bg-navy-800 hover:bg-navy-700 text-navy-300 hover:text-white border border-navy-700 transition-all"
          >
        ...
        <div className="hidden md:flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-navy-800/90 border border-gold-500/30 text-xs font-medium text-navy-100">
          ...
          {agencyLoading ? (
            ...
            <>
              ...
              {currentAgency?.subscriptionPlan && (
                <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gold-500/20 text-gold-300 border border-gold-500/30">
                  {currentAgency.subscriptionPlan}
                </span>
              )}
            </>
