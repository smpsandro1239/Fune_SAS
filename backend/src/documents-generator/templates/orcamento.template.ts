...
export function generateOrcamento(
  funeral: FuneralData,
  agency: AgencyData,
  extra: OrcamentoData,
  sharedDoc?: jsPDF,
  ...
  y = addTitle(doc, 'ORÇAMENTO DE SERVIÇOS FUNERÁRIOS', y);
  y += 4;

  const refNum = `ORC-${Date.now().toString().slice(-6)}`;
  doc.setFontSize(9);
  ...
  ['Serviço', 'Qtd', 'Preço Unit.', 'Subtotal'].forEach((h, i) => doc.text(h, colX[i], y + 2));
  doc.setTextColor(0, 0, 0);
  y += 10;
  ...
  if (extra.paymentMethod) {
    y = addField(doc, 'Condições de Pagamento', extra.paymentMethod, 25, y);
  }
  if (extra.validUntil) {
    y = addField(doc, 'Validade', extra.validUntil, 25, y);
  }
  ...
  y = addSignatureLine(doc, 'Assinatura do Responsável', y, 35);
  y += 8;
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(40, 45, 65);
